...
const page = () => {
  ...
  const getDataStatusProjectById = useQuery({
    queryKey: ["getSkillsById", id],
    queryFn: () => getStatusProjectById({ id }),
  ...
  useMemo(() => {
    if (getDataStatusProjectById.data) {
      form.setValue("name", getDataStatusProjectById?.data?.data?.name);
    }
  }, [getDataStatusProjectById.data]);

  const mutateEditStatusProject = useMutation({
    mutationFn: (payload) =>
      putStatusProject({
        id: id,
        body: payload,
  ...
  const onSubmit = (values) => {
    ...
    mutateEditStatusProject.mutate(formData);
  };
